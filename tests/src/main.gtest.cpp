...
void print_argv(const std::vector<char*>& argv)
{
  std::cout << "argv: [\n";
  for(auto arg : argv)
  {
...
TEST_F(ClaraParserTest, DoubleDelimiterAlias) {
  ...
  auto& optb = p.add_option("option").set_alias("opt").set_alias_options(clara::option_builder::alias_options::double_delimiter);

  auto result = p.parse({ "cmd", "--opt" });
  auto opt = result.root.get_option("option");
  ASSERT_TRUE(opt.has_value()) << "option --option should be set via alias";

//TODO: something liks this
/*
...
TEST_F(ClaraParserTest, SingleDelimiterAlias) {
    ...
    opt.set_alias_options(clara::option_builder::alias_options::single_delimiter);
    opt.set_alias("o");
    ...
    EXPECT_EQ(opt_result.value().get().get_raw(0).value_or("nada"), "file.c") << "Option value should be file.c";
}
...
TEST_F(ClaraParserTest, OptionNoValue) {
    ...
    EXPECT_TRUE(opt->get().get_raw(0).value_or("nada") == "nada") << "Option should have no value";
}
...
TEST_F(ClaraParserTest, OptionWithValue) {
    ...
    EXPECT_EQ(opt->get().get_raw(0).value_or("nada"), "value") << "Option value should be 'value'";
}
...
TEST_F(ClaraParserTest, SubcommandWithValue) {
    ...
    EXPECT_EQ(subcmd->get().get_raw(0).value_or("nada"), "value") << "Subcommand should have value 'value'";
}
...
TEST_F(ClaraParserTest, OptionalEqualsSign) {
    ...
    EXPECT_EQ(opt1->get().get_raw(0).value_or("nada"), "value");
    EXPECT_EQ(opt2->get().get_raw(0).value_or("nada"), "value");
}

TEST_F(ClaraParserTest, StringValues) {
    p.add_option("option").requires_value();
    auto argv1 = make_argv({"sometool", "--option", "'value'"});
    auto argv2 = make_argv({"sometool", "--option='value'"});
    ...
    ASSERT_TRUE(opt1.has_value()) << "Option should be present without =";
    ASSERT_TRUE(opt2.has_value()) << "Option should be present with =";
    EXPECT_EQ(opt1->get().get_raw(0).value_or("nada"), "value");
    EXPECT_EQ(opt2->get().get_raw(0).value_or("nada"), "value");
}
